...
<div class="help-box">Final save will update only the checked batches with the target payment date.</div>
</div>
...
async function confirmScheduleCheckedBatches(){const batch_numbers=checkedBatchNumbers();if(!batch_numbers.length){alert('Select at least one batch.');closeScheduleModal();return}if(!document.getElementById('targetPaymentDate').value){alert('Set the target payment date first.');document.getElementById('targetPaymentDate').focus();return}if(!confirm(`Final save: update ${batch_numbers.length.toLocaleString()} checked batches with this target payment date?`))return;const payload={batch_numbers,target_payment_date:document.getElementById('targetPaymentDate').value,payment_priority:document.getElementById('paymentPriority').value,approval_status:document.getElementById('approvalStatus').value,payment_remarks:document.getElementById('paymentRemarks').value};const res=await fetch('/api/schedule-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(data.error){alert(data.error);return}alert(`Saved schedule for ${Number(data.updated_rows||0).toLocaleString()} checked batches.`);closeScheduleModal();await selectProvider(selectedProvider);await loadMetrics()}
document.getElementById('providerSearch').addEventListener('input',()=>{clearTimeout(providerTimer);providerTimer=setTimeout(loadProviders,300)});document.getElementById('scheduleModal').addEventListener('click',e=>{if(e.target.id==='scheduleModal')closeScheduleModal()});loadMetrics();loadProviders();
</script>
</body></html>
"""
