...
<div><label>Target Payment Date</label><input id="targetPaymentDate" type="date" /></div>
...
<div><button class="good" onclick="scheduleCheckedBatches()">Save Schedule for Checked Batches</button></div>
</div>
...
async function scheduleCheckedBatches(){const batch_numbers=checkedBatchNumbers();if(!batch_numbers.length){alert('Select at least one batch.');return}if(!document.getElementById('targetPaymentDate').value){alert('Select a target payment date.');return}if(!confirm(`Save schedule for ${batch_numbers.length.toLocaleString()} selected batches?`))return;const payload={batch_numbers,target_payment_date:document.getElementById('targetPaymentDate').value,payment_priority:document.getElementById('paymentPriority').value,approval_status:document.getElementById('approvalStatus').value,payment_remarks:document.getElementById('paymentRemarks').value};const res=await fetch('/api/schedule-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(data.error){alert(data.error);return}alert(`Saved schedule for ${Number(data.updated_rows||0).toLocaleString()} selected batches.`);await selectProvider(selectedProvider);await loadMetrics()}
