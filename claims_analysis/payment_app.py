...
def create_app(db_path: str | Path = DEFAULT_DB_PATH) -> Flask:
    ...
    def api_metrics():
        with connect(db_path) as conn:
            row = conn.execute(
                """
                SELECT
                    SUM(CASE WHEN UPPER(COALESCE(payment_status,'')) = 'UNPAID'
                              AND COALESCE(target_payment_date,'') = '' THEN 1 ELSE 0 END) AS for_scheduling_count,
                    SUM(CASE WHEN UPPER(COALESCE(payment_status,'')) = 'UNPAID'
                              AND COALESCE(target_payment_date,'') <> '' THEN 1 ELSE 0 END) AS scheduled_count,
                    SUM(CASE WHEN UPPER(COALESCE(payment_status,'')) = 'UNPAID'
                              AND COALESCE(target_payment_date,'') = ''
                             THEN CAST(COALESCE(NULLIF(expected_check_amount,''),'0') AS REAL) ELSE 0 END) AS for_scheduling_amount,
                    SUM(CASE WHEN UPPER(COALESCE(payment_status,'')) = 'UNPAID'
                              AND COALESCE(target_payment_date,'') <> ''
                             THEN CAST(COALESCE(NULLIF(expected_check_amount,''),'0') AS REAL) ELSE 0 END) AS scheduled_amount
                FROM payment_tags
                """
            ).fetchone()
    ...
    def api_scheduled():
        ...
        params: list[str] = []
        if q:
            search_filter = "AND (provider LIKE ? OR batch_no LIKE ? OR region LIKE ? OR target_payment_date LIKE ?)"
            params.extend([f"%{q}%"] * 4)
        with connect(db_path) as conn:
            rows = conn.execute(
                f"""
                ...
                WHERE UPPER(COALESCE(payment_status,'')) = 'UNPAID'
                  AND COALESCE(target_payment_date,'') <> ''
                  {search_filter}
                ORDER BY target_payment_date ASC, provider ASC, batch_no ASC
                LIMIT 5000
                """,
                params,
            ).fetchall()
...
<div id="selectionSummary" class="note" style="margin-top:10px">Select a provider to view unscheduled unpaid batches.</div>
<div id="treasurySummary" class="treasury-grid"></div>
<div class="split">
<div><div class="note" style="margin-bottom:6px">Providers with unscheduled unpaid batches</div><div id="providerList" class="provider-list"></div></div>
<div><div class="note" style="margin-bottom:6px">Unscheduled unpaid batches under selected provider</div><div id="providerBatchWrap" class="table-wrap"></div></div>
</div>
<div class="action-row">
<div><span id="selectedActionText" class="selected-pill">0 selected</span></div>
<div>
<button class="danger" onclick="clearSelection()">Clear Selection</button>
<button class="good" onclick="openScheduleModal()">Schedule Checked Batches</button>
<button onclick="window.location.href='/export/payments.csv'">Export DB CSV</button>
</div>
</div>
</section>
...
<h3>Scheduled</h3>
<div class="search-row">
<div><label>Search Scheduled</label><input id="scheduledSearch" placeholder="Provider, batch no, region, date..." /></div>
<div><button class="primary" onclick="loadScheduled()">Refresh Scheduled</button></div>
</div>
<div id="scheduledSummary" class="note" style="margin-top:10px">Scheduled batches will appear here.</div>
<div id="scheduledWrap" class="table-wrap" style="margin-top:10px"></div>
<div class="action-row">
<div><span id="scheduledSelectedText" class="selected-pill">0 selected</span></div>
<div>
<button class="danger" onclick="unscheduleChecked()">Return Checked to For Scheduling</button>
<button onclick="window.location.href='/export/payments.csv'">Export DB CSV</button>
</div>
</div>
</section>
...
<div class="modal">
<div class="modal-header"><h3>Schedule Selected Batches</h3></div>
<div class="modal-body">
<div id="modalSummary" class="note" style="margin-bottom:12px">0 batches selected.</div>
...
<div class="help-box">Final save will move checked batches from For Scheduling to Scheduled.</div>
</div>
<div class="modal-footer">
<button onclick="closeScheduleModal()">Cancel</button>
<button class="good" onclick="confirmScheduleCheckedBatches()">Confirm Save Schedule</button>
</div>
</div>
</div>
<script>
let selectedProvider='';let providerBatches=[];let providerTimer=null;let scheduledRows=[];let scheduledTimer=null;
...
function providerStats(){const selected=checkedBatchNumbers();const set=new Set(selected);const totalAmount=providerBatches.reduce((a,r)=>a+num(r.expected_check_amount),0);const selectedAmount=providerBatches.filter(r=>set.has(String(r.batch_no))).reduce((a,r)=>a+num(r.expected_check_amount),0);const dates=providerBatches.map(r=>String(r.date_received||'')).filter(Boolean).sort();const highest=providerBatches.reduce((best,r)=>agingRank(r.aging_bucket)>agingRank(best)?r.aging_bucket:best,'');const credit=[...new Set(providerBatches.map(r=>r.credit_term).filter(Boolean))].join(', ');return{totalBatches:providerBatches.length,totalAmount,selectedCount:selected.length,selectedAmount,remainingCount:providerBatches.length-selected.length,remainingAmount:totalAmount-selectedAmount,oldestDate:dates[0]||'',highestAging:highest||'',creditTerm:credit||''}}
function renderTreasurySummary(){const s=providerStats();const items=[['Provider Outstanding',money(s.totalAmount)],['Unscheduled Batches',s.totalBatches.toLocaleString()],['Selected Amount',money(s.selectedAmount)],['Selected Batches',s.selectedCount.toLocaleString()],['Remaining After Selection',money(s.remainingAmount)],['Remaining Batch Count',s.remainingCount.toLocaleString()],['Oldest Received',s.oldestDate||'-'],['Highest Aging',s.highestAging||'-'],['Credit Term',s.creditTerm||'-']];document.getElementById('treasurySummary').innerHTML=items.map(([l,v])=>`<div class="summary-card"><div class="summary-label">${escapeHtml(l)}</div><div class="summary-value">${escapeHtml(v)}</div></div>`).join('')}
function updateSelectionSummary(){const s=providerStats();document.getElementById('selectionSummary').textContent=`Selected Provider: ${selectedProvider||'None'} | Selected Batches: ${s.selectedCount.toLocaleString()} | Selected Amount: ${money(s.selectedAmount)} | Remaining After Selection: ${money(s.remainingAmount)}`;document.getElementById('selectedActionText').textContent=`${s.selectedCount.toLocaleString()} selected | ${money(s.selectedAmount)}`;renderTreasurySummary()}
...
function updateScheduledSelection(){const selected=checkedScheduledNumbers();const set=new Set(selected);const amount=scheduledRows.filter(r=>set.has(String(r.batch_no))).reduce((a,r)=>a+num(r.expected_check_amount),0);document.getElementById('scheduledSelectedText').textContent=`${selected.length.toLocaleString()} selected | ${money(amount)}`}
async function unscheduleChecked(){const batch_numbers=checkedScheduledNumbers();if(!batch_numbers.length){alert('Select at least one scheduled batch.');return}if(!confirm(`Return ${batch_numbers.length.toLocaleString()} selected batches to For Scheduling?`))return;const res=await fetch('/api/unschedule-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({batch_numbers})});const data=await res.json();if(data.error){alert(data.error);return}alert(`Returned ${Number(data.updated_rows||0).toLocaleString()} batches to For Scheduling.`);await loadScheduled();await loadProviders();await loadMetrics()}
document.getElementById('providerSearch').addEventListener('input',()=>{clearTimeout(providerTimer);providerTimer=setTimeout(loadProviders,300)});document.getElementById('scheduledSearch').addEventListener('input',()=>{clearTimeout(scheduledTimer);scheduledTimer=setTimeout(loadScheduled,300)});document.getElementById('scheduleModal').addEventListener('click',e=>{if(e.target.id==='scheduleModal')closeScheduleModal()});loadMetrics();loadProviders();
</script>
</body></html>
"""
