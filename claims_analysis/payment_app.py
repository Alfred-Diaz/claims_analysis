...
<div class="split"><div><div class="note" style="margin-bottom:6px">Providers with scheduled batches</div><div id="scheduledProviderList" class="provider-list"></div></div><div><div class="view-tabs"><button id="calendarViewBtn" class="view-tab active" onclick="setScheduledView('calendar')">Calendar View</button><button id="listViewBtn" class="view-tab" onclick="setScheduledView('list')">Drilldown List View</button></div><div id="scheduledBatchWrap" class="table-wrap"></div></div></div>
...
<script>
let selectedProvider='';let providerBatches=[];let scheduledProvider='';let scheduledBatches=[];let scheduledView='calendar';let providerTimer=null;let scheduledProviderTimer=null;
...
async function selectScheduledProvider(provider){scheduledProvider=provider;await loadScheduledProviders();const res=await fetch('/api/scheduled-provider-batches?provider='+encodeURIComponent(provider));scheduledBatches=await res.json();scheduledView='calendar';renderScheduledBatches()}
...
function renderScheduledBatches(){if(!scheduledBatches.length){document.getElementById('scheduledBatchWrap').innerHTML='<div style="padding:14px" class="note">No scheduled batches found.</div>';updateScheduledSelection();return}if(scheduledView==='calendar'){renderScheduledCalendar();return}renderScheduledList()}
