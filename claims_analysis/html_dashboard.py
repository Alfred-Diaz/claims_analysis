...
<label>Received / Check Date Range</label><div class="date-row"><input id="dateFromFilter" type="date" /><input id="dateToFilter" type="date" /></div>
<label>Global Search</label><input id="searchInput" placeholder="Batch, check no, CV no, provider, region, any value..." />
...
<footer>Global Search now includes batch number, check number, CV number, provider, supplier category, region, and all other visible values.</footer>
<script>
const DATA = {payload};
const TABLE_LABELS = {{provider_reconciliation:'Provider Totals',results:'All Batches',paid:'Paid',unpaid:'Unpaid',variances:'Batch Variances',for_review:'For Review',duplicate_checks:'Duplicate Checks',duplicate_cv:'Duplicate CV',date_summary:'Date Summary',aging_analysis:'Aging Analysis'}};
const FILE_NAMES = {{provider_reconciliation:'provider_amount_reconciliation.csv',results:'claims_analysis_output.csv',paid:'paid_batches.csv',unpaid:'unpaid_batches.csv',variances:'batch_variances.csv',for_review:'for_review.csv',duplicate_checks:'duplicate_checks.csv',duplicate_cv:'duplicate_cv.csv',date_summary:'date_created_summary.csv',aging_analysis:'aging_analysis.csv'}};
const TABLE_KEYS = ['provider_reconciliation','results','paid','unpaid','variances','for_review','date_summary','aging_analysis','duplicate_checks','duplicate_cv'];
...
function norm(v){{return String(v??'').trim().toLowerCase()}} function upper(v){{return String(v??'').trim().toUpperCase()}} function includesText(v,q){{return !q||norm(v).includes(q)}}
function val(r,c,d=''){{return r[c]??d}} function docType(r){{return upper(val(r,'claim_documentation_type','REGULAR'))||'REGULAR'}} function supplierCategory(r){{return upper(val(r,'supplier_category_name',''))}}
function rowDateValue(r){{return r.date_received||r.received_date||r.date_created||r.created_date||r.check_date||''}}
...
function fillSelect(id, values, allLabel){{const clean=[...new Set(values.map(v=>String(v??'').trim()).filter(Boolean))].sort();document.getElementById(id).innerHTML=`<option value="ALL">${{allLabel}}</option>`+clean.map(v=>`<option value="${{escapeHtml(v)}}">${{escapeHtml(v)}}</option>`).join('')}}
function populateFilters(){{const main=getMainRows();document.getElementById('reportPath').textContent=`Source: ${{DATA.reports_dir}} | Loaded rows: ${{main.length.toLocaleString()}} of ${{(DATA.row_counts.results||main.length).toLocaleString()}}`;fillSelect('providerFilter',main.map(r=>r.provider||'UNKNOWN'),'All Providers');fillSelect('categoryFilter',main.map(r=>supplierCategory(r)),'All Supplier Categories');fillSelect('regionFilter',main.map(r=>r.region),'All Regions');fillSelect('provinceFilter',main.map(r=>r.province),'All Provinces');fillSelect('cityFilter',main.map(r=>r.city),'All Cities');fillSelect('creditTermFilter',main.map(r=>r.credit_term),'All Credit Terms');fillSelect('agingFilter',main.map(r=>r.aging_bucket),'All Aging Buckets');fillSelect('mpsuFilter',main.map(r=>r.mpsu_tag),'All MPSU Tags')}}
function filterValues(){{return {{docType:document.getElementById('docTypeFilter').value,provider:document.getElementById('providerFilter').value,providerText:norm(document.getElementById('providerTextFilter').value),payment:document.getElementById('paymentFilter').value,category:document.getElementById('categoryFilter').value,region:document.getElementById('regionFilter').value,province:document.getElementById('provinceFilter').value,city:document.getElementById('cityFilter').value,creditTerm:document.getElementById('creditTermFilter').value,aging:document.getElementById('agingFilter').value,mpsu:document.getElementById('mpsuFilter').value,recon:document.getElementById('reconFilter').value,dateFrom:document.getElementById('dateFromFilter').value?new Date(document.getElementById('dateFromFilter').value):null,dateTo:document.getElementById('dateToFilter').value?new Date(document.getElementById('dateToFilter').value):null,search:norm(document.getElementById('searchInput').value)}}}}
...
function applyFilters(){{currentPage=1;renderMetrics();renderCharts();renderTabs();renderTable()}} function resetFilters(){{['docTypeFilter','providerFilter','paymentFilter','categoryFilter','regionFilter','provinceFilter','cityFilter','creditTermFilter','agingFilter','mpsuFilter','reconFilter'].forEach(id=>document.getElementById(id).value=id==='docTypeFilter'?'REGULAR':'ALL');['providerTextFilter','dateFromFilter','dateToFilter','searchInput'].forEach(id=>document.getElementById(id).value='');applyFilters()}} function downloadActiveCsv(){{alert(`Open this file from reports/latest: ${{FILE_NAMES[activeTable]}}`)}}
['providerTextFilter','dateFromFilter','dateToFilter','searchInput'].forEach(id=>document.getElementById(id).addEventListener('input',applyFilters));['docTypeFilter','providerFilter','paymentFilter','categoryFilter','regionFilter','provinceFilter','cityFilter','creditTermFilter','agingFilter','mpsuFilter','reconFilter'].forEach(id=>document.getElementById(id).addEventListener('change',applyFilters));populateFilters();applyFilters();
</script>
</body>
</html>"""
