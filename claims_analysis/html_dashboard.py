...
<head>
  ...
  <style>
    :root {{ --bg:#f3f6fb; --card:#ffffff; --text:#172033; --muted:#64748b; --border:#dbe3ef; --primary:#1d4ed8; --danger:#b91c1c; --good:#047857; }}
    * {{ box-sizing: border-box; }}
    ...
    .card {{ background:var(--card); border:1px solid var(--border); border-radius:14px; padding:16px; box-shadow:0 1px 3px rgba(15,23,42,.08); min-width:0; }}
    ...
    .metric-label {{ color:var(--muted); font-size:clamp(11px,1vw,13px); min-height:34px; line-height:1.25; overflow-wrap:anywhere; }}
...
<body>
  ...
  <main><div class=\"layout\">
    <aside class=\"card filters\">
      ...
      <select id=\"docTypeFilter\"><option value=\"REGULAR\">Regular Only</option><option value=\"ALL\">All</option><option value=\"REIMBURSEMENT\">Reimbursement Only</option></select>
    ...
    <section>
      ...
      <section class=\"card\"><div class=\"tabs\" id=\"tabs\"></div><div class=\"toolbar\"><span class=\"note\" id=\"tableNote\"></span><button onclick=\"downloadActiveCsv()\">CSV file name</button></div><div class=\"table-wrap\" id=\"tableWrap\"></div></section>
    </section>
  </div></main>
  <footer>Dashboard generated from enhanced Claims Analysis reports. Regular claims are shown by default; use Documentation Type to include reimbursements.</footer>
<script>
const DATA = {payload};
...
let activeTable = 'provider_reconciliation'; let charts = {{}};
...
function docType(row) {{ return String(row.claim_documentation_type || 'REGULAR').trim().toUpperCase() || 'REGULAR'; }}
function rowDateValue(row) {{ return row.date_created || row.created_date || row.check_date || ''; }}
function parseDate(value) {{ const text = String(value ?? '').split(',')[0].trim(); if (!text || text === 'NO CHECK DATE') return null; const d = new Date(text); return Number.isNaN(d.getTime()) ? null : d; }}
...
function passesDate(row, f) {{ if (!f.dateFrom && !f.dateTo) return true; const d = parseDate(rowDateValue(row)); if (!d) return false; if (f.dateFrom && d < f.dateFrom) return false; if (f.dateTo) {{ const end = new Date(f.dateTo); end.setHours(23,59,59,999); if (d > end) return false; }} return true; }}
...
function groupProviderRowsFromFilteredBatches() {{ const map = new Map(); filterBatchRows(getMainRows()).forEach(r => {{ const provider = r.provider || 'UNKNOWN'; if (!map.has(provider)) map.set(provider, {{provider, batch_count:0, paid_batches:0, unpaid_batches:0, claims_amount:0, withholding_tax:0, expected_check_amount:0, check_amount:0, difference:0, variance_batch_count:0}}); const p = map.get(provider); p.batch_count += 1; if (r.payment_status === 'PAID') p.paid_batches += 1; if (r.payment_status === 'UNPAID') p.unpaid_batches += 1; p.claims_amount += num(r.claims_amount); p.withholding_tax += num(r.withholding_tax); p.expected_check_amount += num(r.expected_check_amount); p.check_amount += num(r.check_amount); p.difference += num(r.difference); if (r.reconciliation_status === 'VARIANCE') p.variance_batch_count += 1; }}); return [...map.values()].map(r => {{ ['claims_amount','withholding_tax','expected_check_amount','check_amount','difference'].forEach(c => r[c] = money(r[c])); return r; }}).sort((a,b)=>num(b.claims_amount)-num(a.claims_amount)); }}
function groupDateRowsFromFilteredBatches() {{ const map = new Map(); filterBatchRows(getMainRows()).forEach(r => {{ const date = rowDateValue(r) || 'NO DATE'; if (!map.has(date)) map.set(date, {{date_created:date, batch_count:0, paid_batches:0, unpaid_batches:0, claims_amount:0, check_amount:0, difference:0}}); const d = map.get(date); d.batch_count += 1; if (r.payment_status === 'PAID') d.paid_batches += 1; if (r.payment_status === 'UNPAID') d.unpaid_batches += 1; d.claims_amount += num(r.claims_amount); d.check_amount += num(r.check_amount); d.difference += num(r.difference); }}); return [...map.values()].map(r => {{ ['claims_amount','check_amount','difference'].forEach(c => r[c] = money(r[c])); return r; }}); }}
...
function renderTable() {{ const tableRows = filteredTableRows(activeTable); const sourceCount = DATA.row_counts[activeTable] || rows(activeTable).length; const loadedCount = rows(activeTable).length; document.getElementById('tableNote').textContent = `Showing ${{tableRows.length.toLocaleString()}} filtered rows. Loaded ${{loadedCount.toLocaleString()}} of ${{sourceCount.toLocaleString()}} source rows. CSV: ${{FILE_NAMES[activeTable]}}`; const wrap = document.getElementById('tableWrap'); if (!tableRows.length) {{ wrap.innerHTML = '<div style="padding:18px" class="note">No rows match the active filters.</div>'; return; }} const cols = Object.keys(tableRows[0]); wrap.innerHTML = `<table><thead><tr>${{cols.map(c=>`<th>${{escapeHtml(c)}}</th>`).join('')}}</tr></thead><tbody>${{tableRows.map(r=>`<tr>${{cols.map(c=>`<td class="${{statusClass(c,r[c])}}">${{escapeHtml(r[c])}}</td>`).join('')}}</tr>`).join('')}}</tbody></table>`; }}
function applyFilters() {{ renderMetrics(); renderCharts(); renderTabs(); renderTable(); }}
function resetFilters() {{ ['docTypeFilter','providerFilter','paymentFilter','categoryFilter','reconFilter'].forEach(id => document.getElementById(id).value = id === 'docTypeFilter' ? 'REGULAR' : 'ALL'); ['providerTextFilter','dateFromFilter','dateToFilter','batchTextFilter','checkTextFilter','cvTextFilter','searchInput'].forEach(id => document.getElementById(id).value=''); applyFilters(); }}
...
['docTypeFilter','providerFilter','paymentFilter','categoryFilter','reconFilter'].forEach(id => document.getElementById(id).addEventListener('change', applyFilters));
